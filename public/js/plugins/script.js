...
function setLayout() {
  var layout = layoutValue; // Retrieve layout data from localStorage
  ...
  if (layout !== null && layout !== '') {
    ...
    if (layout === 'horizontal') {
      ...
      if (document.querySelector('.pc-sidebar .m-header .logo-lg')) {
        document.querySelector('.pc-sidebar .m-header .logo-lg').setAttribute('src', '/images/logo-white.png');
      }
...
document.querySelectorAll('.auth-main.v2 .img-brand').forEach((img) => {
  img.setAttribute('src', '/images/logo-white.png');
});
