...
class DriverController extends Controller
{
    ...
    {
        $tickets = Ticket::whereHas('driver')->filterByRole(request()->user())->with('company')->limit(5)->get();
    ...
    public function index(Request $request)
    {
        ...
        if (request()->ajax()) {
            ...
            $data->getCollection()->transform(function ($item) {
                ...
                $item->action  = '';
                if ($item->roleable) {
                    if (auth()->user()->hasRole('admin') || (auth()->user()->hasRole('manager') && auth()->user()->roleable->canWriteToCompany($item->id))) {
                        $item->action .= '<a href="'.route(auth()->user()->roles->first()->name.'.drivers.edit', $item->roleable->id).'" class="w-8 h-8 rounded-xl inline-flex items-center justify-center btn-link-secondary">
                    ...
                    if (auth()->user()->hasRole('admin')) {
                        $item->action .= '<form action="'.route('admin.drivers.destroy', $item->roleable->id).'" method="POST" class="inline delete-manager-form">
                    ...
                    $item->action .= '
                    <a href="' . route(auth()->user()->roles->first()->name.'.drivers.create', ['ticket_id' => $item->ticket_id]). '" class="w-8 h-8 rounded-xl inline-flex items-center justify-center btn-link-success" title="Register this driver">
